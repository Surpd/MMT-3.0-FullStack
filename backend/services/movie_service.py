...
async def ensure_movie_in_db(movie_id: int, media_type: str = "movie") -> bool:
    ...
    try:   
        ...
        movie_data = {
            ...
            "original_language": tmdb_ext.get("original_language"),
            "backdrop_url": tmdb_ext.get("backdrop_path"),
            "tmdb_vote_count": tmdb_ext.get("vote_count"),
            "production_companies": [c.get("name") for c in tmdb_ext.get("production_companies", []) if c.get("name")],
            "actors": actors,
            "directors": directors,
