...
async def refresh_tv_metadata(tv_id: int, force: bool = False) -> dict[str, Any] | None:
    ...
    if current and current.get("media_type") == "tv" and not force:
        stamp = current.get("metadata_updated_at")
        if stamp:
            try:
                updated = datetime.fromisoformat(str(stamp).replace("Z", "+00:00"))
                if (datetime.now(timezone.utc) - updated).days < TV_METADATA_TTL_DAYS:
                    return current
            except ValueError:
                pass
...
async def get_tv_progress(user_id: int, tv_id: int) -> dict[str, Any]:
    ...
    for season in seasons:
        episodes = await db.get_tv_episodes(tv_id, season["season_number"])
    ...
    caught_up = available_total > 0 and available_watched == available_total
...
async def ensure_tv_structure(tv_id: int, season_count: int | None = None) -> None:
    metadata = await db.get_movie(tv_id) or {}
    count = season_count or metadata.get("seasons") or 0
    for season_number in range(1, int(count) + 1):
