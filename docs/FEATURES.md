...
| Quiz 2.0 | Working / V1 | `QuizTab.tsx` | `/api/quiz/meta`, `/api/quiz?mode=...`, `/api/quiz/answer` | `user_stats`, `movies`, `user_movies`, process-local pool/session cache | shared bounded local pool, rotating library sample, bounded TMDB fallback | cinema/library/daily sessions; indexed generation; answer deltas persist once at completion; no leaderboard |
